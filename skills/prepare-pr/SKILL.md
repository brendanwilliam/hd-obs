...
7. Rebase or otherwise preserve linear history before requesting merge. Resolved review conversations must be
   complete for every protected target. `main` promotions must also pass all required checks; approval is not
   required for the sole maintainer.
