...
bool find_dashboard_item(obs_scene_t *, obs_sceneitem_t *item, void *data)
{
	...
	if (obs_sceneitem_get_source(item) == context->dashboard) {
		...
		auto map_rectangle = [&](int &left, int &top, int &width, int &height) {
			...
			struct vec3 second{float(left + width), float(top + height),
					   0.0f};
			...
			width = std::max(1,
					 int(std::lround(std::abs(second.x - first.x))));
			height = std::max(1,
					  int(std::lround(std::abs(second.y - first.y))));
		};
		map_rectangle(context->left, context->top, context->width,
			      context->height);
		map_rectangle(context->content_left, context->content_top,
...
bool fit_camera_item(obs_scene_t *, obs_sceneitem_t *item, void *data)
{
	...
	if (obs_sceneitem_get_source(item) != context->camera)
		return true;
	context->visibility->fit_item(item, context->left, context->top, context->width,
				      context->height, context->content_left,
				      context->content_top, context->content_width,
...
void lol_dashboard_camera_visibility::fit_to_panel(int left, int top, int width,
						   int height, int content_left,
						   int content_top, int content_width,
						   int content_height)
{
	if (!camera_ || width < 1 || height < 1)
		return;
	fit_context context{this,        dashboard_,    camera_,       left,
			    top,         width,         height,        content_left,
	...
	obs_enum_scenes(
		[](void *data, obs_source_t *scene_source) {
			...
			if (!scene)
				return true;
			fit_context mapped{context->visibility,    context->dashboard,
					   context->camera,        context->left,
					   context->top,           context->width,
					   context->height,        context->content_left,
					   context->content_top,   context->content_width,
					   context->content_height};
...
void lol_dashboard_camera_visibility::fit_item(obs_sceneitem_t *item, int left, int top,
					       int width, int height, int content_left,
					       int content_top, int content_width,
					       int content_height)
{
	const auto saved =
		std::find_if(fitted_items_.begin(), fitted_items_.end(),
	...
	const auto map_coordinate = [](int coordinate, int origin, int extent,
				       uint32_t source_extent) {
		return std::clamp(int(std::lround(double(coordinate - origin) *
						  source_extent / extent)),
				  0, int(source_extent));
	};
	const int crop_left =
		map_coordinate(left, content_left, content_width, source_width);
	const int crop_top =
		map_coordinate(top, content_top, content_height, source_height);
	const int crop_right =
		map_coordinate(left + width, content_left, content_width, source_width);
	const int crop_bottom =
		map_coordinate(top + height, content_top, content_height, source_height);
	const obs_sceneitem_crop crop{crop_left, crop_top,
				      std::max(0, int(source_width) - crop_right),
				      std::max(0, int(source_height) - crop_bottom)};
	obs_sceneitem_set_crop(item, &crop);
	obs_sceneitem_set_visible(item, true);
}

void lol_dashboard_camera_visibility::sync(obs_source_t *dashboard,
					   const std::string &camera_source_uuid)
{
	restore();
	...
	if (!camera || camera == dashboard ||
	    obs_source_get_type(camera) != OBS_SOURCE_TYPE_INPUT ||
