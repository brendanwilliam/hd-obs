...
struct lol_dashboard_rect {
	...
	lol_dashboard_rect adjusted(int left_adjustment, int top_adjustment,
				    int right_adjustment, int bottom_adjustment) const
	{
		return {x_ + left_adjustment, y_ + top_adjustment,
			width_ + right_adjustment - left_adjustment,
...
struct lol_dashboard_panels {
	lol_dashboard_rect header, heatmap, summary, left_stack, keys, camera_mask,
		camera, minimap_cover_mask, minimap_cover;
	bool camera_visible{};
};
...
lol_dashboard_panels lol_dashboard_panel_rectangles(
	...
	const lol_dashboard_image_layout &minimap_cover, int hud_padding = 20);
