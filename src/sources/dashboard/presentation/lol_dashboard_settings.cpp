...
namespace sources {
namespace {
lol_dashboard_regions::widget widget_from_settings(obs_data_t *settings, const std::string &key)
{
	return static_cast<lol_dashboard_regions::widget>(
		std::clamp(int(obs_data_get_int(settings, key.c_str())), 0, 6));
}
lol_dashboard_regions::section section_from_settings(obs_data_t *settings, const char *name,
...
lol_dashboard_font_style lol_dashboard_font_style_from_settings(obs_data *settings, const char *role)
{
	...
	return {QString::fromUtf8(obs_data_get_string(value, (prefix + ".family").c_str())),
		...
		std::clamp(int(obs_data_get_int(value, (prefix + ".size").c_str())), 8, 100),
...
lol_dashboard_regions lol_dashboard_regions_from_settings(obs_data *settings)
{
	...
	result.top = section_from_settings(
		value, "top",
		{true, 2, {lol_dashboard_regions::widget::intensity, lol_dashboard_regions::widget::intensity}});
	result.left = section_from_settings(value, "left",
					    {true,
					     3,
	...
	result.right = section_from_settings(
		value, "right",
		{true, 2, {lol_dashboard_regions::widget::live_keys, lol_dashboard_regions::widget::top_keys}});
	return result;
}
...
lol_dashboard_trail_filter lol_dashboard_trail_filter_from_settings(obs_data *settings)
{
	...
	result.middle_clicks = obs_data_get_bool(value, "lol_dashboard.trail.middle_clicks");
	...
	for (const auto &key :
	     QString::fromUtf8(obs_data_get_string(value, "lol_dashboard.trail.keys")).split(',', Qt::SkipEmptyParts))
