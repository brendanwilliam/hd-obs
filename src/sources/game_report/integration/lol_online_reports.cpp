...
QString payload_hash(QJsonObject value)
{
	...
	return QString::fromLatin1(QCryptographicHash::hash(canonical_payload(value),
...
public:
	explicit implementation(online_reports *owner) : owner(owner), network(owner)
	{
		...
		QObject::connect(device_poll_timer, &QTimer::timeout, owner,
				 [owner] { owner->poll_device_code(); });
		upload_timer->setInterval(1000);
		QObject::connect(upload_timer, &QTimer::timeout, owner,
				 [owner] { owner->tick(); });
	}
	...
	QString root, state{"Not linked. Online reports are disabled."}, device_code,
		token;
...
online_reports::online_reports(QObject *parent)
	: QObject(parent),
	  implementation_(new implementation(this))
{
	implementation_->sessions =
		std::make_unique<session_store>(implementation_->root + "/sessions");
	load_queue();
...
{
	const auto retained = implementation_->sessions
				      ? implementation_->sessions->load()
	...
	if (const auto checkpoint = checkpoints.load_checkpoint()) {
		const bool already_retained = std::any_of(
			retained.cbegin(), retained.cend(),
			[&](const retained_session &session) {
	...
	const auto recovered = implementation_->sessions
				       ? implementation_->sessions->load()
				       : QVector<retained_session>{};
	...
	if (file.open(QIODevice::ReadOnly)) {
		...
		for (const auto entry : saved["queue"].toArray()) {
			...
			if (retained_ids.contains(
				    object["payload"].toObject()["report_id"].toString()))
				implementation_->queue.append(
					{object["payload"].toObject(),
					 QDateTime::fromString(
						 object["retry_at"].toString(),
						 Qt::ISODateWithMs),
	...
	for (const auto &session : recovered) {
		...
		if (session.upload != upload_state::pending ||
		    std::any_of(implementation_->queue.cbegin(),
				implementation_->queue.cend(),
				[&session](const pending_report &entry) {
					return entry.payload["report_id"].toString() ==
					       session.value.id;
				}))
			continue;
		implementation_->queue.append({payload,
					       session.retry_at.isValid()
						       ? session.retry_at
...
void online_reports::save_queue() const
{
	...
	for (auto entry = implementation_->uploaded_payloads.cbegin();
		...
		uploaded.append(QJsonObject{{"id", entry.key()},
					    {"payload_hash", entry.value()}});
	QSaveFile file(implementation_->root + "/online-upload-queue.json");
	if (file.open(QIODevice::WriteOnly)) {
		file.write(QJsonDocument(QJsonObject{{"schema_version", 2},
						     {"queue", values},
						     {"uploaded", uploaded}})
...
{
	QProcess process;
	process.start("/usr/bin/security", {"find-generic-password", "-w", "-a",
					    account_name, "-s", service_name});
...
bool online_reports::save_credential(const QString &value) const
{
	...
	process.start("/usr/bin/security",
		      {"add-generic-password", "-U", "-a", account_name, "-s",
		       service_name, "-w", value});
...
{
	QProcess process;
	process.start("/usr/bin/security", {"delete-generic-password", "-a", account_name,
					    "-s", service_name});
...
void online_reports::submit(const report &value)
{
	...
	if (implementation_->sessions)
		implementation_->sessions->save({value, upload_state::pending,
	...
	if (!found)
		implementation_->queue.append(
			{payload, QDateTime::currentDateTimeUtc(), 0});
	save_queue();
}
...
void online_reports::set_upload_enabled(bool enabled)
{
	...
	if (!enabled)
		implementation_->state =
			"Uploads are disabled. Completed reports remain local only.";
	else if (linked())
...
{
	if (implementation_->shutting_down || !implementation_->upload_enabled ||
	    !linked() || implementation_->auth_required ||
	...
	connect(reply, &QNetworkReply::finished, this, [this, reply] {
		...
		if (!implementation_->queue.isEmpty() &&
		    reply->error() == QNetworkReply::NoError && code >= 200 &&
		    code < 300 &&
		    (result_status == "accepted" || result_status == "duplicate")) {
			const QJsonObject uploaded =
				implementation_->queue.first().payload;
			if (implementation_->sessions)
				implementation_->sessions->update_upload(
					uploaded["report_id"].toString(),
					upload_state::confirmed, {},
					implementation_->queue.first().attempts);
			implementation_->queue.removeFirst();
			implementation_->uploaded_payloads.insert(
				uploaded["report_id"].toString(),
				uploaded["payload_hash"].toString());
			implementation_->state =
				implementation_->queue.isEmpty()
					? "Connected. All reports uploaded."
					: "Connected. Uploading reports.";
		} else if (!implementation_->queue.isEmpty() &&
			   result_status == "rejected") {
			implementation_->state =
				"Upload rejected. Update Hands Diff before retrying this report.";
			implementation_->queue.first().retry_at =
				QDateTime::currentDateTimeUtc().addYears(10);
			if (implementation_->sessions)
				implementation_->sessions->update_upload(
					implementation_->queue.first()
						.payload["report_id"]
						.toString(),
			...
			entry.retry_at = QDateTime::currentDateTimeUtc().addSecs(
				1 << qMin(entry.attempts, 8));
			if (implementation_->sessions)
				implementation_->sessions->update_upload(
					entry.payload["report_id"].toString(),
					upload_state::pending, entry.retry_at,
					entry.attempts);
			implementation_->state =
				"Upload delayed; it will retry automatically.";
		}
		save_queue();
		reply->deleteLater();
	});
...
void online_reports::begin_link()
{
	if (implementation_->shutting_down || !implementation_->device_code.isEmpty())
		return;
	QNetworkRequest request(
		implementation_->service_url.resolved(QUrl("/api/device/start")));
	request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
	auto *reply =
		implementation_->network.post(request, R"({"client_name":"Hands Diff"})");
	connect(reply, &QNetworkReply::finished, this, [this, reply] {
		...
		if (!implementation_->device_code.isEmpty() && verification.isValid() &&
		    !code.isEmpty()) {
			implementation_->state =
				QString("Approve online reports in your browser with code %1.")
					.arg(code);
			implementation_->next_device_poll =
				QDateTime::currentDateTimeUtc().addSecs(
					value["interval"].toInt(5));
			implementation_->device_code_expires_at =
				QDateTime::currentDateTimeUtc().addSecs(
					value["expires_in"].toInt(600));
			...
			implementation_->state =
				"Could not start online linking. Please try again.";
		}
		reply->deleteLater();
	});
}
...
void online_reports::poll_device_code()
{
	...
	if (implementation_->device_code_expires_at.isValid() &&
	    implementation_->device_code_expires_at <= now) {
	...
	QNetworkRequest request(
		implementation_->service_url.resolved(QUrl("/api/device/token")));
	request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
	auto *reply = implementation_->network.post(
		request,
		QJsonDocument(QJsonObject{{"device_code", implementation_->device_code}})
			.toJson());
	connect(reply, &QNetworkReply::finished, this, [this, reply] {
...
void online_reports::retry()
{
	...
	implementation_->state =
		implementation_->queue.isEmpty()   ? "No queued reports to retry."
